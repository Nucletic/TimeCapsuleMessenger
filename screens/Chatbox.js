...
import { collection, doc, onSnapshot, updateDoc, increment, Timestamp, query, where, getDocs, addDoc, orderBy, limit, endBefore } from 'firebase/firestore'
...
const Chatbox = ({ navigation, route }) => {
  ...
  const { chatId, username, profileImage, blockedFromOther, blockedFromOur, onlineStatus, lastOnline, ExpoPushToken } = route.params;
  ...
  const sendMobileNotification = async (Type, message, chatId, username, profileImage, blockedFromOther, blockedFromOur, onlineStatus, lastOnline, ExpoPushToken) => {
    try {
      ...
      const response = await fetch(`https://server-production-3bdc.up.railway.app/users/sendNotification`, {
        ...
        body: JSON.stringify({ Type, message, chatId, username, profileImage, blockedFromOther, blockedFromOur, onlineStatus, lastOnline, ExpoPushToken })
      })
